...
class GAGSCalculator {
     ...
     */
    fun determineRegion(centerX: Float, centerY: Float): FaceRegion? {
    ...
    fun calculateGAGSScore(detections: List<ImageAnalyzer.Detection>): Int {
        ...
        FaceRegion.values().forEach { region ->
        ...
        regionDetections.forEach { (region, regionDetections) ->
            ...
            if (regionDetections.isEmpty()) {
                ...
                Log.d(TAG, "Region: ${region.name}, Lesion: NO_LESION, " +
                ...
                if (mostSevereLesion != null) {
                    ...
                    Log.d(TAG, "Region: ${region.name}, Lesion: ${lesionType.name}, " +
